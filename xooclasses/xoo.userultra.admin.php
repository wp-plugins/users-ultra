...
class XooUserAdmin extends XooUserUltraCommon 
{
	...
	function add_menu() 
	{
		...
		add_menu_page( __('Users Ultra','userultra'), $menu_label, 'manage_options', $this->slug, array(&$this, 'admin_page'), xoousers_url .'admin/images/small_logo_16x16.png', '199.150');
	...
	public function get_special_checks($tab) 
	{
		...
		if($tab=="settings")
		{
		 ...
		 $special_with_check = array('hide_admin_bar', 'private_message_system','redirect_backend_profile','redirect_backend_registration','redirect_backend_login', 'social_media_fb_active', 'social_media_linked_active', 'social_media_yahoo', 'social_media_google', 'mailchimp_active', 'media_allow_photo_uploading');
	...
	function checkUploadFolder()
	{
		...
		if(!is_dir($path_pics))
		{
			...
			$html .= ' <div ><div class="user-ultra-warning">'.__("Please create '".$media_folder."' folder with 0777 attribute.", 'xoousers').'</div></div>';
			...
			if($f_perm !=777)
			{
				...
				$html .= ' <div ><div class="user-ultra-warning">'.__("Change attributes TO 0777 for  '".$media_folder."' folder.", 'xoousers').'</div></div>';	
